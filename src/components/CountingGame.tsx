...
export function CountingGame({ onClose, addStars, showToast, playSound, advanceQuest, difficulty = 'easy' }: CountingGameProps) {
    ...
    return (
      <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-b from-green-400 to-emerald-600 p-6 rounded-3xl">
        ...
        <button
          ...
          className="bg-lime-400 border-4 border-black text-black px-8 py-4 rounded-full font-black text-xl uppercase shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:bg-lime-500 transition-colors"
        >
          Back to Games
        </button>
      </div>
  ...
  return (
    <div className="w-full h-full flex flex-col bg-gradient-to-b from-sky-300 to-sky-500 rounded-3xl overflow-hidden relative border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
      ...
      <div className="flex-1 relative mx-4 mb-4 bg-white/20 border-4 border-black rounded-3xl overflow-hidden shadow-inner min-h-0">
        <AnimatePresence>
          {items.map(item => (
            <motion.div
              key={item.id}
      ...
      <div className="grid grid-cols-3 gap-3 md:gap-4 px-4 pb-4 shrink-0">
        {options.map(opt => {
          let btnClass = 'bg-lime-400 border-4 border-black text-black hover:bg-lime-300';
          if (answered) {
            if (opt === targetCount) btnClass = 'bg-green-500 border-4 border-black text-white scale-105';
          ...
          return (
            <motion.button
              ...
              disabled={answered}
              className={`py-4 md:py-5 rounded-2xl font-black text-3xl md:text-4xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all ${btnClass}`}
            >
              {opt}
            </motion.button>
          );
