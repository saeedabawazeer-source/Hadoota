...
import { GAME_HOSTS } from './gameHosts';
...
export interface GameConfig {
  ...
  color: string;
  host?: string;          // pet id hosting this subject (see data/gameHosts.ts)
  makeRound: (difficulty: string, streak?: number) => Round;
}

// Millionaire-style climb: the trivia subjects (Science/Geography) start at
// the kid's set difficulty and ramp up a tier every 3 correct answers in a
// row, capped at "hard" — the ladder-climbing feel without ever starting
// harder than what the parent configured.
const TIERS = ['easy', 'medium', 'hard'];
export function escalate(base: string, streak: number): string {
  const baseIdx = Math.max(0, TIERS.indexOf(base));
  const bump = Math.min(TIERS.length - 1 - baseIdx, Math.floor(streak / 3));
  return TIERS[baseIdx + bump];
}
...
export const GAME_CONFIGS: Record<string, GameConfig> = {
  'Math Dash':   { key: 'Math Dash',   title: 'Math School',  label: 'Drive to the answer',  emoji: '➕', color: '#9333ea', host: GAME_HOSTS['Math Dash'],   makeRound: mathRound },
  'Counting':    { key: 'Counting',    title: 'Counting Yard', label: 'Count them!',          emoji: '🔢', color: '#a3e635', host: GAME_HOSTS['Counting'],    makeRound: countingRound },
  'Spelling':    { key: 'Spelling',    title: 'Word Wagon',    label: 'Find the letter',      emoji: '🔤', color: '#3b82f6', host: GAME_HOSTS['Spelling'],    makeRound: wordsRound },
  'Shapes':      { key: 'Shapes',      title: 'Shape Studio',  label: 'Find it!',             emoji: '🔺', color: '#f97316', host: GAME_HOSTS['Shapes'],      makeRound: shapesRound },
  'Memory Quiz': { key: 'Memory Quiz', title: 'Animal Arcade', label: 'Find the animal',      emoji: '🐾', color: '#f59e0b', host: GAME_HOSTS['Memory Quiz'], makeRound: animalsRound },
  'Science':     { key: 'Science',     title: 'Science Lab',   label: 'Pick the answer',      emoji: '🔬', color: '#ec4899', host: GAME_HOSTS['Science'],
    makeRound: (d, streak = 0) => bankRound(scienceQuestions, escalate(d, streak)) },
  'Geography':   { key: 'Geography',   title: 'World Tower',   label: 'Pick the answer',      emoji: '🌍', color: '#14b8a6', host: GAME_HOSTS['Geography'],
    makeRound: (d, streak = 0) => bankRound(geographyQuestions, escalate(d, streak)) },
};
